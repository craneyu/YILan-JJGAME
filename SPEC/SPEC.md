...
| 項目     | 說明                                              |
| -------- | ------------------------------------------------- |
| 比賽類型 | 傳統演武（本規格書主要對象；創意演武的計分規則見 [4.5](#45-創意演武計分規則)） |
| 組別     | 男子組、女子組、混合組                            |
...
> 例：MALE R2-G3 = 男子組第二輪（B 系列）、第三組出賽隊伍。
> 例：FEMALE R1-G1 = 女子組第一輪（A 系列）、第一組出賽隊伍。
> 女子組與混合組每輪僅演練各系列 1–3 個動作（如 A1–A3）。
> 組別標籤：MALE（男子）、FEMALE（女子）、MIXED（混合）。
...
| R2-G1    | Round 2（B 系列）第 1 組出賽隊伍；輪次對應系列：R1=A、R2=B、R3=C；組別會顯示在前面（如 MALE R2-G1）                           |
| 組別標籤  | 各裁判頁面的 Round 標籤會顯示組別前綴：MALE（男子）、FEMALE（女子）、MIXED（混合）                                            |
| 錯誤攻擊  | VR 裁判可於該系列全部動作完成後，標記某些動作為「錯誤攻擊」（無分數），影響最終排名計算                                      |
...
創意演武（Show）與傳統演武共用平台但計分方式獨立，不分系列與動作，每隊演練一次。
本節僅規範計分與排名；其餘流程（計時、報到等）依系統實作。

#### 4.5.1 裁判評分

5 位計分裁判各自送出**技術分**與**表演分**，兩項各 0–9.5 分、以 0.5 為間隔。

各項分別去掉最高分與最低分，加總中間 3 位，得到扣分前的：

- 技術分原始分（最高 28.5 = 3 × 9.5）
- 表演分原始分（最高 28.5）
- 原始總分（最高 57）

#### 4.5.2 違例扣分歸屬

扣分**不從總分一次扣除**，而是扣在所屬的那一項分數：

| 違例         | 扣分 | 扣在   | 說明                                           |
| ------------ | ---- | ------ | ---------------------------------------------- |
| 超時         | 1.0  | 表演分 | 超過規定演練時間                               |
| 未達時間     | 1.0  | 表演分 | 未達規定演練時間                               |
| 使用道具     | 1.0  | 表演分 | 道具超過 2 樣；是否成立由裁判現場判斷，成立即扣一次 |
| 未達攻擊次數 | 0.5  | 技術分 | 攻擊次數未達要求                               |
...
```
技術分 = max(0, 技術分原始分 − 技術分扣分)
表演分 = max(0, 表演分原始分 − 表演分扣分)
最終得分 = 技術分 + 表演分
```

兩項各自不低於 0。原始總分保留作為對帳用，以下兩條等式皆成立：
...
同組別（錦標賽另依分級）內依**最終得分**由高至低排名。

- **平分時以技術分（扣分後）較高者為優先**
- 下列隊伍不列入排名：棄權、未滿 5 位裁判送出評分（該隊總分一律為 0）

#### 4.5.5 觀眾端排名顯示

觀眾端於當前隊伍評分結算後，顯示該隊所屬組別的排名表，供現場對照：
...
- 分組條件與計分端一致：同組別、同分級（錦標賽）
- 表格下方註明「最終得分相同時，以技術分較高者排前」，讓平分結果可被理解
- 另顯示當前隊伍名次 `RANK #n / m`，其中 m 只計列入排名的隊伍；
  未列入排名者顯示「未列入排名」
...
- 賽後各組別分別匯出賽果：
  - **Excel（.xlsx）**：詳細 PART 分數表，各動作逐項列出（P1–P5）、VR 分數、系列合計
  - **PDF**：排版適合列印或簽核，包含各組別排名、每隊成績、裁判長簽名欄位
- 刪除賽事：支援刪除整場賽事及所有相關評分資料
- 設定各組適用系列與動作數量
- 管理輪次（Round）與出賽順序

### 5.2 計分裁判功能

- 頁面頂端顯示組別標籤與輪次（例：MALE R2-G3）
- 賽序裁判開放評分後，評分操作區解鎖
...
- 頁面頂端顯示組別標籤與輪次（例：MIXED R1-G2）
- 顯示當前系列選手的動作清單與已完成狀態
- 已完成的動作可點擊按鈕標記為「錯誤攻擊」（標記後顯示紅色警告標籤）
- 全系列動作完成後，開放摔技與地板技多樣性評分區（各 0–2 分）
- 選分完成後「確認送出」按鈕可點按
- 送出後進入唯讀狀態，顯示已送出評分結果
- **全螢幕模式**：點擊頁面按鈕隱藏瀏覽器界面
- 等待賽序裁判換組信號
...
- 即時顯示當前隊伍組別標籤、累計總分與各動作分項分數
- 即時顯示當前動作各評分項目的 5 位裁判加總（P1–P4 / P1–P5）
- 顯示 VRT（VR 多樣性小計）及 TOT（系列合計）
- 顯示各組別排名（同一組別內排序）
- 未完成動作欄位以「`-`」表示
- 標記為「錯誤攻擊」的動作會顯示紅色警告標籤
- **全螢幕模式**：點擊頁面按鈕隱藏瀏覽器界面（網址列、工具列、分頁），最大化競賽顯示面積
