...
export class SequenceJudgeComponent implements OnInit, OnDestroy {
  ...
  judgeStatuses = signal<JudgeStatus[]>(
    [1, 2, 3, 4, 5].map((n) => ({ judgeNo: n, submitted: false })),
  );
  vrSubmitted = signal(false);
  vrScoreValues = signal<{
    throwVariety: number;
  ...
  // Computed
  all5Submitted = computed(() =>
    this.judgeStatuses().every((j) => j.submitted),
  );
  ...
  allActionsDone = computed(
    () =>
      ...
      this.actionProgress().every((a) => a.status === "done"),
  );
  seriesTotalScore = computed(() => {
    const actionsTotal = this.actionProgress().reduce(
      (sum, a) => sum + (a.actionTotal ?? 0),
      0,
    );
  ...
  nextPendingAction = computed(
    () =>
      this.actionProgress().find((a) => a.status === "pending")?.actionNo ??
      null,
  );
  ...
  hasMultipleTypes = computed(
    () => this.auth.eventCompetitionTypes().length > 1,
  );
  currentTypeName = computed(() =>
    this.auth.competitionType() === "creative" ? "創意演武" : "雙人演武",
  );
  otherTypeName = computed(() =>
    this.auth.competitionType() === "creative" ? "雙人演武" : "創意演武",
  );

  switchCompetitionType(): void {
    const newType =
      this.auth.competitionType() === "creative" ? "kata" : "creative";
    this.auth.setCompetitionType(newType);
    this.router.navigate([
      newType === "creative" ? "/creative/sequence" : "/judge/sequence",
    ]);
  }
    ...
    this.subs.push(
      ...
      this.socket.scoreSubmitted$.subscribe((e) => {
        this.judgeStatuses.update((s) =>
          s.map((j) =>
            j.judgeNo === e.judgeNo ? { ...j, submitted: true } : j,
          ),
        );
        ...
        this.judgeScores.update((scores) => {
          ...
          return [
            ...filtered,
            { judgeNo: e.judgeNo, items: e.items as JudgeScoreEntry["items"] },
      ...
      this.socket.wrongAttackUpdated$.subscribe(
        (e: WrongAttackUpdatedEvent) => {
          this.actionProgress.update((p) =>
            p.map((a) =>
              a.actionNo === e.actionNo
                ? {
      ...
      this.socket.vrSubmitted$.subscribe((e) => {
        ...
        this.vrScoreValues.set({
          throwVariety: e.throwVariety,
          groundVariety: e.groundVariety,
        });
      }),
      ...
      this.socket.groupChanged$.subscribe((e) => {
        ...
        if (prevTeam && prevRound === 3) {
          if (!nextTeam || nextTeam.category !== prevTeam.category) {
            ...
            const prevCatTeams = this.teams().filter(
              (t) => t.category === prevTeam.category,
            );
        ...
        if (nextTeam) {
          ...
          this.groupIndex.set(
            this.teams().findIndex((t) => t._id === e.nextTeamId) + 1,
          );
  ...
  loadSummary(eventId: string): void {
    this.api
      .get<SummaryResponse>(`/events/${eventId}/summary`)
      .subscribe((res) => {
        ...
        if (gameState.status === "event_complete") {
          ...
          const currentCat = teams.find(
            (t) => t._id === gameState.currentTeamId,
          )?.category;
          ...
          if (gameState.currentRound === 3 && currentCat) {
            ...
            const currentIdx = sameCatTeams.findIndex(
              (t) => t._id === gameState.currentTeamId,
            );
            for (let i = 0; i < currentIdx; i++)
              completedIds.add(sameCatTeams[i]._id);
          }
        ...
        if (vrScore || gameState.status === "series_complete") {
          ...
          if (vrScore) {
            this.vrScoreValues.set({
              throwVariety: vrScore.throwVariety,
              groundVariety: vrScore.groundVariety,
            });
          }
        }
        ...
        if (gameState.currentTeamId) {
          ...
          if (team) {
            ...
            this.groupIndex.set(
              teams.findIndex((t) => t._id === gameState.currentTeamId) + 1,
            );
            ...
            if (completedActionNos.length > 0) {
              const scoreMap = new Map(
                calculatedScores?.map((s) => [s.actionNo, s]) ?? [],
              );
            ...
            if (gameState.currentActionNo && gameState.currentActionOpen) {
              ...
              this.judgeStatuses.update((s) =>
                s.map((j) => ({
                  ...j,
                  submitted: submittedJudgeNos.includes(j.judgeNo),
                })),
              );
              // 還原個別裁判分數（當前開放動作）
              if (currentActionJudgeScores?.length) {
                this.judgeScores.set(
                  [...currentActionJudgeScores].sort(
                    (a, b) => a.judgeNo - b.judgeNo,
                  ),
                );
              ...
              const lastCompleted =
                completedActionNos[completedActionNos.length - 1];
              if (
                lastCompleted &&
                completedActionJudgeScores?.[lastCompleted]?.length
              ) {
                this.judgeScores.set([
                  ...completedActionJudgeScores[lastCompleted],
                ]);
  ...
  setAbstain(): void {
    this.api
      .post<{ success: boolean }>("/flow/abstain", { eventId: this.eventId() })
      .subscribe({
        next: (res) => {
          if (res.success) {
            this.teamAbstained.set(true);
  ...
  cancelAbstain(): void {
    this.api
      .post<{
        success: boolean;
      }>("/flow/cancel-abstain", { eventId: this.eventId() })
      .subscribe({
        next: (res) => {
          if (res.success) {
            this.teamAbstained.set(false);
