...
export class AdminComponent implements OnInit {
  ...
  duoTeams = computed(() =>
    this.teams().filter((t) => (t.competitionType ?? "Duo") !== "Show"),
  );
  showTeams = computed(() =>
    this.teams().filter((t) => t.competitionType === "Show"),
  );
  ...
  loadTeams(eventId: string): void {
    this.api
      .get<{ success: boolean; data: TeamItem[] }>(`/events/${eventId}/teams`)
      .subscribe({
        next: (res) => {
  ...
  addTeam(): void {
    ...
    if (!event) return;
    const members = [this.newTeam.member1, this.newTeam.member2].filter(
      (m) => m.trim() !== "",
    );
    if (!this.newTeam.name || members.length === 0) {
      Swal.fire({
        icon: "warning",
        title: "請填寫隊伍名稱與至少一位隊員",
        toast: true,
        position: "top-end",
    ...
    this.api
      ...
      .subscribe({
        next: (res) => {
          if (res.success) {
            this.teams.update((t) =>
              [...t, res.data].sort((a, b) => a.order - b.order),
            );
            this.newTeam = {
              name: "",
              member1: "",
  ...
  saveEdit(team: TeamItem): void {
    const event = this.selectedEvent();
    if (!event) return;
    const members = [this.editForm.member1, this.editForm.member2].filter(
      (m) => m.trim() !== "",
    );
    ...
    this.api
      .patch<{ success: boolean; data: TeamItem }>(
        `/events/${event._id}/teams/${team._id}`,
        {
      ...
      .subscribe({
        next: (res) => {
          if (res.success) {
            this.teams.update((t) =>
              t
                .map((tm) => (tm._id === team._id ? res.data : tm))
  ...
  async deleteTeam(team: TeamItem): Promise<void> {
    ...
    this.api
      .delete<{
        success: boolean;
      }>(`/events/${this.selectedEvent()?._id}/teams/${team._id}`)
      .subscribe({
        next: () => {
  ...
  saveReorder(): void {
    ...
    const map = this.reorderMap();
    const orders = this.filteredTeams().map((t) => ({
      id: t._id,
      order: map[t._id] ?? t.order,
    }));
    ...
    this.api
      .post<{
        success: boolean;
        data: TeamItem[];
      }>(`/events/${eventId}/teams/batch-order`, { orders })
      .subscribe({
        next: (res) => {
          if (res.success) {
            this.teams.set(res.data);
  ...
  async batchDeleteTeams(): Promise<void> {
    ...
    this.api
      .post<{
        success: boolean;
        data: { deleted: number };
      }>(`/events/${this.selectedEvent()!._id}/teams/batch-delete`, { ids })
      .subscribe({
        next: (res) => {
          ...
          Swal.fire({
            icon: "success",
            title: `已刪除 ${res.data.deleted} 支隊伍`,
            toast: true,
            position: "top-end",
            showConfirmButton: false,
            timer: 2000,
  ...
  importFileForType(event: Event, competitionType: "Duo" | "Show"): void {
    ...
    this.api
      ...
      }>(`/events/${this.selectedEvent()!._id}/teams/import`, formData)
      .subscribe({
        next: (res) => {
          if (res.success) {
  ...
  clearScoresByType(type?: "Duo" | "Show"): void {
    ...
    const typeName =
      type === "Duo" ? "雙人演武" : type === "Show" ? "創意演武" : "所有";
    const typeHtml =
      type === "Duo"
        ? "<b>雙人演武</b>的計分與 VR 裁判資料"
        : type === "Show"
          ? "<b>創意演武</b>的計分與扣分資料"
          : "<b>所有</b>競賽項目的計分資料";
  ...
  loadCreativeRankings(): void {
    ...
    this.api
      .get<{
        success: boolean;
        data: CreativeRankingItem[];
      }>(`/events/${event._id}/creative-rankings`)
      .subscribe({
        next: (res) => {
          if (res.success) {
...
<div class="sign-area">
  ...
  exportExcel(category: string): void {
    ...
    // 工作表標題
    rows.push([`${event.name} — ${group.label} 成績明細`]);
    merge(0, COL - 1);
    rows.push([`列印日期：${new Date().toLocaleDateString("zh-TW")}`]);
    merge(0, COL - 1);
    rows.push([]);
        ...
        for (let i = 1; i <= actionCount; i++) {
          ...
          for (let p = 1; p <= 5; p++) {
            r.push(
              p <= parts
                ? d
                  ? ((d as Record<string, number>)[`p${p}`] ?? 0)
                  : 0
                : "",
            );
          }
...
<div class="sign-area">
  ...
  effectiveCategoryOrder(
    ...
    const specific =
      type === "Duo" ? event.categoryOrderDuo : event.categoryOrderShow;
    if (specific && specific.length > 0) return specific;
    return event.categoryOrder?.length
      ? event.categoryOrder
      : ["female", "male", "mixed"];
  }
  ...
  startEditCategoryOrder(event: EventItem): void {
    ...
    this.categoryOrderDraftDuo.set([
      ...this.effectiveCategoryOrder(event, "Duo"),
    ]);
    this.categoryOrderDraftShow.set([
      ...this.effectiveCategoryOrder(event, "Show"),
    ]);
    const types = event.competitionTypes ?? ["Duo"];
  ...
  moveCategoryOrder(index: number, direction: -1 | 1): void {
    ...
    const arr = [
      ...(isShow
        ? this.categoryOrderDraftShow()
        : this.categoryOrderDraftDuo()),
    ];
  ...
  saveCategoryOrder(): void {
    ...
    if (types.includes("Duo"))
      body["categoryOrderDuo"] = this.categoryOrderDraftDuo();
    if (types.includes("Show"))
      body["categoryOrderShow"] = this.categoryOrderDraftShow();
    this.api
      .patch<{
        success: boolean;
        data: EventItem;
      }>(`/events/${eventId}/category-order`, body)
      .subscribe({
        next: (res) => {
          if (res.success) {
  ...
  // ── 柔術賽程匯入 ──────────────────────────────────────
  matchImportLoading = signal(false);
  ...
  downloadMatchTemplate(): void {
    ...
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "柔術賽程範本");
    XLSX.writeFile(wb, "柔術賽程匯入範本.xlsx");
  }

  importMatchFile(event: Event): void {
    const file = (event.target as HTMLInputElement).files?.[0];
    const ev = this.selectedEvent();
    if (!file || !ev) return;

    this.matchImportLoading.set(true);
    ...
    reader.onload = (e) => {
      try {
        ...
        const rows: Array<Record<string, string>> = XLSX.utils.sheet_to_json(
          ws,
          { defval: "" },
        );
        ...
        const matches = rows.map((row, i) => {
          const matchType =
            TYPE_MAP[
              String(row["項目"] ?? row["matchType"] ?? "")
                .toLowerCase()
                .trim()
            ];
          const category =
            CAT_MAP[
              String(row["組別"] ?? row["category"] ?? "")
                .toLowerCase()
                .trim()
            ];
          return {
            matchType: matchType ?? "ne-waza",
            category: category ?? "male",
            weightClass: String(row["量級"] ?? row["weightClass"] ?? "").trim(),
            round: Number(row["回合"] ?? row["round"] ?? 1),
            matchNo: i + 1,
            scheduledOrder: Number(
              row["場次序"] ?? row["scheduledOrder"] ?? i + 1,
            ),
            redPlayer: {
              name: String(
                row["紅方姓名"] ?? row["redPlayerName"] ?? "",
              ).trim(),
              teamName: String(
                row["紅方隊名"] ?? row["redTeamName"] ?? "",
              ).trim(),
            },
            bluePlayer: {
              name: String(
                row["藍方姓名"] ?? row["bluePlayerName"] ?? "",
              ).trim(),
              teamName: String(
                row["藍方隊名"] ?? row["blueTeamName"] ?? "",
              ).trim(),
            },
            isBye: false,
          };
        ...
        this.api
          .post<{
            ...
            count: number;
            error?: string;
          }>(`/events/${ev._id}/matches/bulk`, matches)
          .subscribe({
            next: (res) => {
              this.matchImportLoading.set(false);
              (event.target as HTMLInputElement).value = "";
              if (res.success) {
                Swal.fire({
                  icon: "success",
                  title: `成功匯入 ${res.count} 筆賽程`,
                  toast: true,
                  position: "top-end",
                  showConfirmButton: false,
                  timer: 2500,
